    ...
    >
      <ol className="flex flex-1 flex-col justify-center gap-3.5">
        {stages.map((s, i) => {
          ...
          return (
            <li key={s.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-x-4 gap-y-1.5">
              ...
              <div className="col-span-2 h-2.5 overflow-hidden rounded-full bg-ivory-sunken">
                <div
                  ...
                  style={{
                    ...
                    background: focal ? '#FF6B1A' : i === last ? '#FF9A5C' : '#B9B3A8',
                  }}
                />
              </div>
            </li>
      ...
      <div className="mt-6 flex items-baseline justify-between border-t border-hairline pt-4">
        ...
        <span className="num text-[22px] font-semibold text-cid-deep">{fmtPct(stages[last - 1].value / top)}</span>
      </div>
    </Card>
  )
}
